...
public class EntityExcalibur extends EntityBeam {
    ...
    public void tick() {
        ...
        if(this.world.isRemote) {
            for(int i = 0; i < 2; i++)
                this.world.addParticle(new ColoredParticleData(ModParticles.light.get(), 245 / 255F, 245 / 255F, 5 / 255F, 1, 2), this.hitVec.getX(), this.hitVec.getY(), this.hitVec.getZ(), this.rand.nextGaussian() * 0.007, this.rand.nextGaussian() * 0.007 + 0.003, this.rand.nextGaussian() * 0.007);
            ...
            for (int i = 0; i < 4; i++) {
                ...
                this.world.addParticle(new ColoredParticleData(ModParticles.light.get(), 245 / 255F, 245 / 255F, 5 / 255F, 1, 0.15f), ppos.getX(), ppos.getY(), ppos.getZ(), this.rand.nextGaussian() * 0.01, this.rand.nextGaussian() * 0.01, this.rand.nextGaussian() * 0.01);
            }
        }
    }
