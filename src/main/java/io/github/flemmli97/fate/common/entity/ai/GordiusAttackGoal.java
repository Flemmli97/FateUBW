...
public class GordiusAttackGoal extends AnimatedAttackGoal<EntityGordius> {

    private double[] chargeMotion;

    public GordiusAttackGoal(EntityGordius entity) {
        super(entity);
    }
    ...
    public void handleAttack(AnimatedAction animatedAction) {
        ...
        if (animatedAction.getTick() >= animatedAction.getAttackTime()) {
            ...
            for (LivingEntity e : list) {
                if (e != this.attacker) {
                    e.attackEntityFrom(CustomDamageSource.gordiusTrample(this.attacker, (LivingEntity) this.attacker.getPassengers().get(0)), Config.Common.gordiusDmg);
                }
            }
