...
public class RenderMedusaDagger extends EntityRenderer<EntityDaggerHook> {
    ...
    public RenderMedusaDagger(EntityRendererManager renderManager) {
        super(renderManager);
    }

    @Override
    public void render(EntityDaggerHook entityIn, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
        LivingEntity playerentity = entityIn.getOwner();
        if (playerentity != null) {
            ...
            if ((this.renderManager.options == null || this.renderManager.options.getPointOfView().func_243192_a()) && playerentity == Minecraft.getInstance().player) {
                ...
                d5 = playerentity.prevPosY + playerentity.getEyeHeight() + (playerentity.getPosY() - playerentity.prevPosY) * partialTicks - 0.45D;
    ...
    @Override
    public ResourceLocation getEntityTexture(EntityDaggerHook entity) {
        return BOBBER;
    }
}
