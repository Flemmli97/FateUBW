...
public class EntityPegasus extends EntityCreature{
	...
	public boolean attackEntityFrom(DamageSource source, float amount) {
		if(this.isBeingRidden() && source!=DamageSource.outOfWorld)
		{
	...
	public void onUpdate() {
		if(this.getRidingEntity()!=null && this.getRidingEntity() instanceof EntityLiving)
		{
			...
			Entity target = rider.getAITarget();
			if(target!=null)
			{
	...
	static class FlyingMoveHelper extends EntityMoveHelper
	{
		...
		private boolean isNotColliding(double x, double y, double z, double p_179926_7_)
        {
            ...
            for (int i = 1; (double)i < p_179926_7_; ++i)
            {
                ...
                if (!this.pegasus.worldObj.getCollisionBoxes(this.pegasus, axisalignedbb).isEmpty())
                {
                    return false;
                }
            }
