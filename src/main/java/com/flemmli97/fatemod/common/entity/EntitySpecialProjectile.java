...
public abstract class EntitySpecialProjectile extends Entity{
    ...
    public void setHeadingToPosition(double x, double y, double z, float velocity, float inaccuracy)
    {
		...
		this.setThrowableHeading(dir.x, dir.y, dir.z, velocity, inaccuracy);
    }
    ...
    public void onUpdate()
    {
        ...
        if(!rayTrace)
        {
	        ...
	        Entity entity = null;
	        List<Entity> list = this.world.getEntitiesWithinAABBExcludingEntity(this, this.getEntityBoundingBox().expand(this.motionX, this.motionY, this.motionZ).grow(this.range+1.0D));
	        boolean flag = false;
	        ...
	        for (int i = 0; i < list.size(); ++i)
	        {
	            ...
	            if (entity1.canBeCollidedWith())
	            {
	                ...
	                else
	                {
	                		...
	                		axisalignedbb = axisalignedbb.grow(range);
	                		if(!piercing)
	                		{
    ...
    public RayTraceResult entityRayTrace(float range)
    {
		...
		Vec3d posEye = new Vec3d(posThrower.x, posThrower.y + thrower.getEyeHeight(), posThrower.z);
		Vec3d look = posEye.add(thrower.getLookVec().scale(range));
		RayTraceResult blockpos = this.world.rayTraceBlocks(posEye, look, true, true, false);
		if(blockpos!=null)
		{
			return blockpos;
		}
		...
		{
			RayTraceResult res = new RayTraceResult(RayTraceResult.Type.MISS, look, EnumFacing.getFacingFromVector((float)look.x, (float)look.y, (float)look.z),null);
			return res;
		}
    }
