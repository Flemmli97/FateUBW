...
public class EntityEnumaElish extends EntitySpecialProjectile{

    protected static final DataParameter<Double> hitX = EntityDataManager.<Double>createKey(EntityEnumaElish.class, CustomDataPacket.Double);
    protected static final DataParameter<Double> hitY = EntityDataManager.<Double>createKey(EntityEnumaElish.class, CustomDataPacket.Double);
    protected static final DataParameter<Double> hitZ = EntityDataManager.<Double>createKey(EntityEnumaElish.class, CustomDataPacket.Double);
    protected static final DataParameter<Float> rotPitch = EntityDataManager.<Float>createKey(EntityEnumaElish.class, DataSerializers.FLOAT);
    protected static final DataParameter<Float> rotYaw = EntityDataManager.<Float>createKey(EntityEnumaElish.class, DataSerializers.FLOAT);
    ...
    public EntityEnumaElish(World worldIn, EntityLivingBase shooter)
    {
        ...
        this.dataManager.set(hitX, hitResult.hitVec.x);
        this.dataManager.set(hitY, hitResult.hitVec.y);
        this.dataManager.set(hitZ, hitResult.hitVec.z);
    }
	...
	public void onUpdate() {
		...
		if(hitResult!=null)
		{
			...
			List<Entity> list = this.world.getEntitiesWithinAABBExcludingEntity(this, 
					new AxisAlignedBB(Math.min(this.posX,this.hitResult.hitVec.x+scaler.x),Math.min(this.posY,this.hitResult.hitVec.y+scaler.y),Math.min(this.posZ,this.hitResult.hitVec.z+scaler.z), 
							Math.max(this.posX,this.hitResult.hitVec.x+scaler.x),Math.max(this.posY,this.hitResult.hitVec.y+scaler.y),Math.max(this.posZ,this.hitResult.hitVec.z+scaler.z)));
			for (int i = 0; i < list.size(); ++i)
	        {
	            ...
	            if (entity1.canBeCollidedWith())
	            {
	            		if(!list.isEmpty() && entity1 instanceof EntityLivingBase && !this.ignore.contains(entity1))
	        			{
	            			...
	            			AxisAlignedBB axisalignedbb = living.getEntityBoundingBox().grow(range);
	...
	protected void onImpact(RayTraceResult result) {
		if (!this.world.isRemote)
        {
            ...
            {
    				result.entityHit.attackEntityFrom(CustomDamageSource.excalibur(this, this.getThrower()), 13.0F);
            }
        }
	}
