        	...
        	if(capSync.getServant() != null || msg.command == 8)
        	{
	        	...
	        	else if(msg.command == 3)
	        	{
	        		if(!capSync.getServant().forcedNP)
	        		{
	        			...
	        			{
	        				if(capSync.useMana(player, capSync.getServant().getSpecialMana()) && capSync.useCommandSeal(player))
	        				{
	        					player.addChatMessage(new TextComponentString(TextFormatting.RED + "You commanded your servant to use a Nobel Phantasm"));	        							
	        					capSync.getServant().forcedNP = true;
	        					capSync.useCommandSeal(player);
	        				}
	        				else
	        				{
	        					player.addChatMessage(new TextComponentString(TextFormatting.RED + "Seems like you don't have enough mana or command spells"));
	        				}
	        			}
	        			else
	        			{
					...
					{
						player.addChatMessage(new TextComponentString(TextFormatting.RED + "Your servant is already preparing for an attack"));
					}
	        	}
        		//follow
	        	...
	        	{
	        		capSync.getServant().attackEntityFrom(DamageSource.outOfWorld, Float.MAX_VALUE);
	        		capSync.setServant(null);
	        		player.addChatMessage(new TextComponentString(TextFormatting.RED + "You killed your servant"));
	        	}
        		//forfeit
	        	...
	        	else if(msg.command==10)
	        	{
	        		if(capSync.useCommandSeal(player))
	        		{
		        		...
		        		player.addChatMessage(new TextComponentString(TextFormatting.RED + "You buffed your servant using a command spell"));
	        		}
	        		else
		        		player.addChatMessage(new TextComponentString(TextFormatting.RED + "You don't have any command spells anymore"));
	        	}
        	}
			return null;
