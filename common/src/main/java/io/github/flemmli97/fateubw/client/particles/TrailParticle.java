...
public class TrailParticle extends TextureSheetParticle {
    ...
    protected TrailParticle(ClientLevel level, SpriteSet sprite, double x, double y, double z, TrailInfo trail) {
        super(level, x, y, z);
        this.trail = trail;
        ...
        this.setSprite(sprite.get(level.getRandom()));
    }
    ...
    public void render(VertexConsumer buffer, Camera camera, float partialTicks) {
        ...
        for (int i = 0; i < position.size(); i++) {
            ...
            if (previous == null)
                previous = pos;
            TrailPositions.TrailPosition previouesTwo = position.getAt(-i - 2);
            if (previouesTwo == null)
                previouesTwo = previous;
            Vector4f[] vertices = this.vertices(pos, previous, previouesTwo, i, partialTicks, position.size());
            for (Vector4f vert : vertices) {
                vert.transform(mat);
            }

            float prog = Mth.clamp((float) (i) / position.size(), 0, 1);
            float progPre = Mth.clamp((float) (i + 1) / position.size(), 0, 1);
    ...
    protected void draw(VertexConsumer buffer, Vector4f[] vertices, float u0, float u1, float v0, float v1, float r, float g, float b, float a, float r2, float g2, float b2, float a2, int light) {
        buffer.vertex(vertices[0].x(), vertices[0].y(), vertices[0].z()).uv(u0, v1).color(r2, g2, b2, a2).uv2(light).endVertex();
        buffer.vertex(vertices[1].x(), vertices[1].y(), vertices[1].z()).uv(u0, v0).color(r2, g2, b2, a2).uv2(light).endVertex();
        buffer.vertex(vertices[2].x(), vertices[2].y(), vertices[2].z()).uv(u1, v0).color(r, g, b, a).uv2(light).endVertex();
        buffer.vertex(vertices[3].x(), vertices[3].y(), vertices[3].z()).uv(u1, v1).color(r, g, b, a).uv2(light).endVertex();
    }

    private Vector4f[] vertices(TrailPositions.TrailPosition current, TrailPositions.TrailPosition last, TrailPositions.TrailPosition lastTwo, int count, float partialTicks, int length) {
        float prog = Mth.clamp((count + partialTicks), 0, length) / length;
        float progPre = Mth.clamp((count + 1 + partialTicks) / length, 0, 1);
        float progPreTwo = Mth.clamp((count + 2 + partialTicks) / length, 0, 1);

        float scale = Mth.lerp(prog, this.trail.width, this.trail.width2) * 0.2f;
        float scalePre = Mth.lerp(progPre, this.trail.width, this.trail.width2) * 0.2f;
        float scalePreTwo = Mth.lerp(progPreTwo, this.trail.width, this.trail.width2) * 0.2f;
